...
define(function () {
	...
	return {
		...
		setDateRangeOnDetails: function (dateRange) {
			try {
				if (this.breakPoint <= EVENT_CONSTANS.BREAKPOINT.TABLET) {
					this.view.flexDateTM.setVisibility(true);
					this.view.lblDateValueTM.text = dateRange;
				} else {
					this.view.lblDateValuePrev.text = dateRange;
					this.view.flexDateTM.setVisibility(false);
				}
		...
		setLocation: function (location) {
			try {
				if (this.breakPoint <= EVENT_CONSTANS.BREAKPOINT.TABLET) {
					this.view.lblLocationValuePrevTM.text = location;
				} else {
		 ...
		 */
		setLocationOnMap: function (lat, long, address, visibility) {
			if (!visibility) {
				this.view.flexGoogleMapTM.setVisibility(false);
				this.view.flexGoogleMap.setVisibility(false);
			...
			try {
				...
				if (this.breakPoint <= EVENT_CONSTANS.BREAKPOINT.TABLET) {
					this.view.mapGoogleMapTM.locationData = [
						locData
					];
					this.view.flexGoogleMapTM.setVisibility(true);
				} else {
					this.view.mapGoogleMap.locationData = [
						locData
					];
					this.view.flexGoogleMapTM.setVisibility(false);
					this.view.flexGoogleMap.setVisibility(true);
				}
		...
		setLocationTitle: function (title) {
			if (this.breakPoint <= EVENT_CONSTANS.BREAKPOINT.TABLET) {
				this.view.flexLocationPrevTM.setVisibility(true);
				this.view.lblLocationPrevTbM.text = title;
			} else {
				this.view.flexLocationPrevTM.setVisibility(false);
				this.view.lblLocationPrev.text = title;
			}
			this.view.forceLayout();
		 ...
		 */
		setEventDetails: function (eventDetails, breakpoint) {
			try {
				...
				if (eventDetails.event_type == "2") {
					if (eventDetails.location_processed !== undefined) {
						...
						this.setLocationTitle("Event Location");
						this.setLocationOnMap(eventDetails.location[0].latitude, eventDetails.location[0].longitude, eventDetails.location_processed, true);
					} else {
						this.setLocationTitle("");
						this.setLocationOnMap("", "", "", false);
					}
				} else {
					this.setLocationOnMap("", "", "", false);
					this.setLocationTitle("Online Event");
					this.setLocation(eventDetails.location_processed);
				}
				if (eventDetails.event_sessions !== undefined && eventDetails.event_sessions.length) {
					this.view.lblSessionTitleDet.isVisible = true;
					var response = {};
					for (var session of eventDetails.event_sessions) {
						session.presenter = eventDetails.presenter.filter((obj) => {
								if (session.event_session_id == obj.session_id)
									return obj;
							});
					}
					response.records = JSON.parse(JSON.stringify(eventDetails.event_sessions));
		...
		createSessionPreviewComponentDynamically: function (id, data) {
			try {
				var sessionPreview = new com.konyenb.sessiondetails({
